...
export function FeynmanGuardian() {
  ...
  const handleSubmit = () => {
    if (explanation.length < 20) {
      setError("Muito breve. Explique como se estivesse ensinando para uma criança de 12 anos.");
      return;
    }
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          >
            <div className="bg-gradient-to-r from-amber-500/20 to-orange-500/20 p-6 border-b border-white/5 flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">O Portão de Feynman</h2>
                <p className="text-amber-200/60 text-xs uppercase tracking-widest font-bold">Verificando Domínio</p>
              </div>
            </div>

            <div className="p-6 space-y-4">
              <p className="text-slate-300">
                Para marcar <span className="text-white font-bold">"{node?.data.label}"</span> como Dominado, você precisa provar que entende.
              </p>

              <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                <label className="text-xs text-slate-500 font-bold uppercase mb-2 block">Sua Explicação Simples</label>
                <textarea
                  value={explanation}
                  onChange={(e) => setExplanation(e.target.value)}
                  placeholder="Explique este conceito de forma simples, sem usar jargão..."
            ...
            <div className="p-4 bg-black/20 flex justify-end gap-3">
              ...
              >
                <CheckCircle2 size={16} /> Confirmar Domínio
              </button>
            </div>
          </motion.div>
        </div>
