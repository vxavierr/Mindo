        ...
        >
            ...
            <div className="h-16 border-b border-slate-200 dark:border-white/5 flex justify-between items-center px-8 bg-white dark:bg-[#050505]">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold text-slate-700 dark:text-white tracking-tight">Via Neural Ativa</span>
                </div>
                <div className="flex items-center gap-4">
                    <span className="text-xs text-slate-400 font-mono hidden md:block">Pressione ESC para fechar</span>
                ...
                <div className="flex-[0.7] flex flex-col border-r border-slate-200 dark:border-white/5 bg-white dark:bg-transparent overflow-y-auto custom-scrollbar">
                    <div className="max-w-4xl w-full mx-auto p-10 pb-4">
                        <input
                            ...
                            placeholder="Neurônio Sem Título"
                        />
                        ...
                        <div className="flex flex-wrap gap-2 items-center mb-6">
                            ...
                            >
                                ...
                                {isSuggesting ? 'Pensando...' : 'Adicionar Tag'}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 px-4 max-w-4xl w-full mx-auto">
                        <CompetenceBlur nodeId={nodeId}>
                            <NeuralEditor
                                nodeId={nodeId}
                                initialContent={node.data.content || '<p>Comece a conectar seus pensamentos...</p>'}
                ...
                <div className="flex-[0.3] bg-slate-50/80 dark:bg-[#02040A] flex flex-col min-h-0 overflow-y-auto border-l border-white/5">
                    ...
                    <div className="p-6 border-b border-slate-200 dark:border-white/5">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-2">
                                <BrainCircuit size={14} /> Unidades de Memória
                            </h3>
                        ...
                        <div className="space-y-3">
                            {memoryUnits.length > 0 ? (
                                ...
                                <div className="py-6 px-4 border-2 border-dashed border-slate-200 dark:border-white/5 rounded-xl text-center">
                                    <p className="text-xs text-slate-400">Nenhuma unidade de memória detectada.</p>
                                    <p className="text-[10px] text-slate-500 mt-1">Selecione texto no editor para criar.</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="p-6 border-b border-slate-200 dark:border-white/5">
                        <h3 className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                            <Share2 size={14} /> Rede Sináptica
                        </h3>

                        <div className="mb-4">
                            <EdgeListSection label="ENTRADAS" icon={ArrowDownLeft} items={incomingEdges} />
                        </div>

                        <div>
                            <EdgeListSection label="SAÍDAS" icon={ArrowUpRight} items={outgoingEdges} />
                        </div>
                    </div>
                    ...
                    <div className="p-6 mt-auto bg-slate-100/50 dark:bg-white/[0.02]">
                        <div className="flex flex-col gap-2 text-[10px] text-slate-400 font-mono mb-4">
                            <div className="flex items-center gap-2"><Clock size={10} /> Criado: {format(new Date(node.data.created_at || new Date()), 'dd MMM yyyy HH:mm')}</div>
                            <div className="flex items-center gap-2"><Clock size={10} /> Atualizado: {format(new Date(node.data.updated_at || new Date()), 'dd MMM yyyy HH:mm')}</div>
                        ...
                        <button className="w-full bg-gradient-to-r from-indigo-600 to-violet-600 text-white px-4 py-3 rounded-xl text-sm font-bold shadow-lg hover:shadow-indigo-500/30 transition-all flex items-center justify-center gap-2">
                            <Sparkles size={16} className="fill-white/20" /> Aprimorar com IA
                        </button>
                    </div>
                </div>
            </div>
