...
export function CanvasToolbar() {
  ...
  // Derive unique tags for the menu
  const uniqueTags = Array.from(new Set(nodes.flatMap(n => n.data.tags || []))).slice(0, 5); // Limit to 5 for UI
  ...
  // If in selection mode, show a different toolbar
      ...
      <AnimatePresence>
        {activeMenu === 'create' && (
          <motion.div
            ...
            className="absolute bottom-16 bg-white dark:bg-mindo-depth border border-slate-200 dark:border-white/10 rounded-xl shadow-2xl p-2 min-w-[150px] flex flex-col gap-1"
          >
            <button 
            ...
            >
              <div className="p-1 bg-indigo-500/10 rounded text-indigo-500"><Plus size={14} /></div>
              New Node
            </button>
          </motion.div>
        )}
