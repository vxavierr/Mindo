...
export function SocraticEdge({
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      if (label.trim()) {
        ...
        cancelEdge(id); // Empty label kills the edge
      }
  ...
  const handleBlur = () => {
    // If user clicks away without saving, the edge is destroyed (Mandatory Rule)
    if (!label.trim()) {
      cancelEdge(id);
    }
  };
    ...
    <>
      <path
        ...
        style={{
            ...style,
            strokeWidth: 2,
            stroke: '#c084fc', // Accent color (Purple)
            strokeDasharray: '5, 5',
            animation: 'dash 1s linear infinite', // Visual indicator of "Waiting"
        }}
