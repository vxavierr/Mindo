...
function CanvasContent() {
  // Use custom hook for keyboard shortcuts (Spacebar, Escape)
  useCanvasShortcuts();

  // OPTIMIZATION: Use useShallow to prevent re-renders when unrelated store parts change
  ...
  const { 
    ...
    organizeGraph, deleteNodes, toggleSelectionDraft 
  } = useMindoStore(state => ({
  ...
  // Memoize canvas nodes to avoid recalculation on every render
  ...
  // Click Handler - Fork behavior based on Mode
  ...
  return (
    <div className={`h-full w-full relative bg-[#F1F5F9] dark:bg-mindo-void overflow-hidden transition-colors duration-500 ${isHandMode ? 'cursor-grab active:cursor-grabbing' : ''}`} onClick={() => setMenu(null)}>
      ...
      {menu && <ContextMenu {...menu} onDelete={(ids) => { deleteNodes(ids); setMenu(null); }} onEdit={(id) => { openEditor(id); setMenu(null); }} onRename={() => setMenu(null)} onClose={() => setMenu(null)} />}
    </div>
  );
}
