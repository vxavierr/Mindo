  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 flex flex-col gap-1 overflow-y-auto no-scrollbar min-h-0">
        <NavItem to="/" icon={Activity} label="Visão Geral" collapsed={isSidebarCollapsed} />
        <NavItem to="/canvas" icon={LayoutGrid} label="Canvas Neural" collapsed={isSidebarCollapsed} />
        <NavItem to="/review" icon={BrainCircuit} label="Revisão Diária" collapsed={isSidebarCollapsed} />
      </nav>
      ...
      <div className="mt-auto border-t border-slate-200 dark:border-white/5 pt-4 space-y-2 shrink-0">
        ...
        <button
          ...
          title="Captura Rápida (Cmd+K)"
        >
          <Zap size={20} className="text-mindo-glow group-hover:scale-110 transition-transform" />
          {!isSidebarCollapsed && <span className="text-sm font-medium text-slate-600 dark:text-slate-300 group-hover:text-indigo-900 dark:group-hover:text-white">Captura Rápida</span>}
        </button>

        <NavItem to="/settings" icon={Settings} label="Configurações" collapsed={isSidebarCollapsed} />
        ...
        >
          ...
          {!isSidebarCollapsed && <span className="text-xs font-bold uppercase tracking-wider">{theme === 'dark' ? 'Modo Claro' : 'Modo Escuro'}</span>}
        </button>
        ...
        >
          {isSidebarCollapsed
            ...
            : <div className="flex items-center gap-2 text-[10px] uppercase tracking-widest opacity-60"><ChevronLeft size={14} /> Recolher</div>
          }
        </button>
      </div>
    </motion.aside>
